...
inputString  = 'W:\SiMView2\13-12-30\Pha_E1_H2bRFP_01_20131230_140802.corrected';
sourceString = 'X:\SiMView2\13-12-30\Pha_E1_H2bRFP_01_20131230_140802.corrected\Results\MultiFused\Pha_E1_H2bRFP';
outputString = 'X:\SiMView2\13-12-30\Pha_E1_H2bRFP_01_20131230_140802.corrected\Results\TimeFused\Pha_E1_H2bRFP';
inputID      = '_blending';
outputID     = '_blending';
lookUpTable  = 'X:\SiMView2\13-12-30\Pha_E1_H2bRFP_01_20131230_140802.corrected\Scripts\SPM00_CM00_CM01_CHN00_CHN01_analyzeParameters\lookUpTable.mat';
...
if ~isempty(timepoints)
    ...
    if jobMemory(1) == 1 && localRun(1) ~= 1
        inputFolder = [inputString '\SPM' num2str(specimen, '%.2d') '\TM' num2str(timepoints(1), '%.6d')];
        header = ['SPM' num2str(specimen, '%.2d') '_TM' num2str(timepoints(1), '%.6d') ...
            '_CM' num2str(cameras(1), '%.2d') '_CHN' num2str(tChannels(1), '%.2d')];
        fileName = [inputFolder '\' header inputExtension];
    ...
    if jobMemory(1) ~= 2 && localRun(1) ~= 1
        ...
        parameterDatabase = [pwd '\jobParameters.timeFuse.' timeString '.mat'];
        ...
        if localRun(1) ~= 1
            for t = 1:nTimepoints
                inputFolder = [inputString '\SPM' num2str(specimen, '%.2d') '\TM' num2str(timepoints(t), '%.6d')];
