...
input_parameters.dimensions   = []; %[];                  % override parameter for user-provided raw stack dimensions (ImageJ-x, ImageJ-y, z/t)
...
input_parameters.startsLeft   = [ 50  50];         % cropping left start coordinates for each camera (ImageJ-x convention)
input_parameters.startsTop    = [  68   55];         % cropping top start coordinates for each camera (ImageJ-y convention)
input_parameters.widths       = [1978 1978];         % cropping width for each camera (ImageJ-w convention)
input_parameters.heights      = [1978 1978];         % cropping height for each camera (ImageJ-h convention)
...
input_parameters.startsFront  = [   0    0];         % cropping front start coordinates for each camera (ImageJ-y convention)
input_parameters.depths       = [];         % cropping depth for each camera (ImageJ-h convention)

input_parameters.inputType    = 0;                   % 0: input data in TIFF format
...
input_parameters.segmentFlag  = 1;                   % 0: do not remove background in output stacks
...
input_parameters.thresholds   = [0.4 0.4];           % adaptive thresholds for each reference channel group
...
input_parameters.verbose      = 1;                   % 0: do not display processing information, 1: display processing information
